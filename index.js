...
const url = `mongodb+srv://admin:admin123@cluster0.agbdf3v.mongodb.net/?retryWrites=true&w=majority`;
...
app.get("/filter/price/:item", (req, res) => {
  ...
  if (lcost && hcost) query = { new_price: { $gt: lcost, $lt: hcost } };
  else if (!lcost && hcost) query = { new_price: { $gt: 50, $lt: hcost } };
  else if (lcost && !hcost) query = { new_price: { $gt: lcost } };
...
app.delete("/cart/delete/:email/:item_type/:item_id", (req, res) => {
  ...
  db.collection("cart").deleteOne(
    { email: emailId, item_id: itemId, item_type: itemType },
    (err, result) => {
      if (err) throw err;
      // res.send(result);
      if (Number(result.deletedCount) === 0) {
        ...
        res
          .status(200)
          .send(
            `Item no: ${itemId}, type: ${itemType}, of user ${emailId} deleted !\n Delete Count: ${result.deletedCount}`
          );
      }
    }
  );
...
app.delete("/wishlist/delete/:email/:item_type/:item_id", (req, res) => {
  ...
  let itemType = req.params.item_type;
  let itemId = Number(req.params.item_id);
  db.collection("wishlist").deleteOne(
    { email: emailId, item_type: itemType, item_id: itemId },
    (err, result) => {
      if (err) throw err;
      // res.send(result);
      if (Number(result.deletedCount) === 0) {
...
app.post("/orders/add", (req, res) => {
  ...
  let transactionState = req.body.transaction_state
    ? req.body.transaction_state
    : "In Progress";
